...
if parallel_computing==0:
    ...
    for i in range(len(problems)):
        ...
        models.append(t)
    print("Exercise 2 with MLP training finished in %f."%(time.clock()-time_start))
    time_start = time.clock()
    ...
    print("Exercise 2 with MLP predicting finished in %f."%(time.clock()-time_start))
else:
    time_start = time.clock()
    problems = divide_problem(20000,20000,data_only=1)
    flat_ploblems=[]
    ...
    for i in range(len(problems)):
        ...
        for k in range(len(t)):
            p_val[k]=max(p_val[k],t[k])
    print("Exercise 2 with MLP training and predicting finished in %f."%(time.clock()-time_start))
TPR=[]
FPR=[]
...
pyplot.xlabel("FPR")
pyplot.ylabel("TPR")
pyplot.title("ROC of Exercise 2 with MLP")
pyplot.savefig("roc_exercise_2_with_mlp.pdf")
#pyplot.show()
