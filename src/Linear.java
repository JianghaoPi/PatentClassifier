...
public class Linear {
    ...
    public static double patentPredict(Model model, Feature[] x, double threshold) {
        double value = patentPredictValue(model, x);
        if (value > threshold) return 1;
        if (value < -threshold) return -1;
        return 0;
    }

    public static double patentPredictValue(Model model, Feature[] x) {
        int n;
        if (model.bias >= 0)
        ...
        // info("dec_values[0] is %g\n", dec_values[0]);
        if (dec_values[0] > 0) {
            return model.label[0] * dec_values[0];
        } else {
            return -model.label[1] * dec_values[0];
        }
    }
